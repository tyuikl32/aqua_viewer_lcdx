...
function getTaskStatus(ctx, platformInput = null) {
  ...
  if (active.stale || !taskDir || !existsSync(taskDir)) {
    return `Status: STALE POINTER\nTask: ${taskRef}\nNext-Action: Task directory not found. Run: py -3 ./.trellis/scripts/task.py finish`
  }
...
function buildCompactCurrentState(ctx, platformInput, specIndexPaths) {
  ...
  if (existsSync(tasksDir)) {
    try {
      ...
      lines.push(`Active tasks: ${activeTasks.length} total. Use \`py -3 ./.trellis/scripts/task.py list --mine\` only if needed.`)
    } catch {
